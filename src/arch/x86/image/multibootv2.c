	...
	* type of 0 */
	while (i < BOOT_INFO_BUFFER_SIZE) {
		...
		if (tag->type == 0 && tag->size == 0) {
			// We add the new tag size to the total size of our boot info
			// structure
			start_tag->total_size += padded_tag_size;

			// We increment the pointer behind the pointer to point to current
			// new tag
			*curr_tag_local_ptr += i;
			...
			if (tag->size == 0) {
				DBG("Found tag with invalid size of zero at addr 0x%lx\n",
					virt_to_phys(tag));
				DBG("tag hex: 0x%llx\n", *(uint64_t *)tag);
				return -EINVAL;
			}
...
static int multiboot_build_memmap(struct image *image, uint8_t **tag_ptr) {
	...
	if (((rc = add_tag_entry(tag_ptr, sizeof(struct multiboot_memory_info_tag),
	...
	if (((rc =
			  add_tag_entry(tag_ptr, mem_tag_size, MULTIBOOT_TAG_TYPE_MMAP)))) {
	...
	for (i = 0; i < memmap.count; i++) {
		...
		uint8_t *new_tag_addr =
			...
			+sizeof(struct multiboot_memory_map_tag) // We jump over struct
													 // multiboot_memory_map_tag
...
static int multiboot_parse_header_tags(struct image *image,
	...
	copy_from_user(&tag, image->data,
				   hdr->offset + sizeof(struct multiboot_header), sizeof(tag));
	...
	for (int i = 0; i < MAX_HEADER_TAGS; i++) {
		...
		case MULTIBOOT_HEADER_TAG_END:
			if (tag.size != 8) {
				DBGC(image,
					 "MULTIBOOT2 %p header end tag has to have size 8 is "
					 "however %d\n",
					 image, tag.size);
				return -EINVAL;
			}
		...
		default:
			if (tag.flags != 0) {
				DBGC(image,
					 "MULTIBOOT2 %p header has unsupported header tag %#x that "
					 "is "
					 "required\n",
					 image, tag.type);
				return -ENOTSUP;
		...
		if (hdr->offset + tag.size >= image->len) {
			DBGC(image,
				 "MULTIBOOT2 %p header has invalid size in tag, this exceeds "
				 "the image."
				 "Tag size: %x\n",
				 image, tag.size);
			return -EINVAL;
		...
		copy_from_user(&tag, image->data, hdr->offset + tag.size, sizeof(tag));
	}
...
static int multiboot_load_raw(struct image *image,
	...
	if (hdr->tags.entry_addr == 0) {
		DBGC(image,
			 "MULTIBOOT2 %p raw image does not have needed entry addr header "
			 "tag\n",
			 image);
		return -EINVAL;
	}
	...
	filesz = (hdr->tags.load_end_addr
				  ...
				  : (image->len - offset));
	memsz =
		(hdr->tags.bss_end_addr ? (hdr->tags.bss_end_addr - hdr->tags.load_addr)
								: filesz);
...
static int multiboot_exec(struct image *image) {
	...
	// Add end tag
	if (((rc = add_tag_entry(&tag_ptr, sizeof(struct multiboot_bootinfo_header),
